...
void __init at91_add_device_usbh(struct at91_usbh_data *data)
{
	...
	for (i = 0; i < data->ports; i++) {
		if (data->vbus_pin[i]) {
			// Update USB: active low -> active high
			// SP2526A-2 -> SP2526A-1
...
printk(KERN_INFO "at91_i2c: at91_add_device_i2c()->GPIO %d\n", nr_devices);
...
// We applied this driver at91_i2c for mulitple i2c devices
// By the comments from GPL community,
// We need to use this driver carefully:
// => do not use high clock rate
// => do not run it too frequent
//   so that prevent rx overflow and tx overflow
// currently, we use I2C devices,
// 1. RTC
// 2. Light Sensor
// 3. Touch sensitive keyboard
// We reduce the rate of hit in order to prevent driver overflow
...
void __init at91_add_device_i2c(struct i2c_board_info *devices, int nr_devices)
{
	...
	i2c_register_board_info(0, devices, nr_devices);
printk(KERN_INFO "at91_i2c: at91_add_device_i2c %d\n", nr_devices);
	platform_device_register(&at91sam9263_twi_device);
}
#else
void __init at91_add_device_i2c(struct i2c_board_info *devices, int nr_devices) {}
...
void __init at91_add_device_ac97(struct atmel_ac97_data *data)
{
	...
	// AC97 => update ALC203 with new RESET pin
	// 1 => stop active low RESET
	at91_set_gpio_output(AT91_PIN_PD13, 1);
...
void __init at91_add_device_lcdc(struct atmel_lcdfb_info *data)
{
	...
	/* Configure LCD pins */
    at91_set_gpio_output(AT91_PIN_PC3, 1); // PIN_CS
	at91_set_gpio_output(AT91_PIN_PC28, 1); // PIN_SCK
	at91_set_gpio_output(AT91_PIN_PC29, 0); // PIN_DI
...
// Start
	//at91_set_gpio_output(AT91_PIN_PD7, 0);	//default receive
	//at91_set_gpio_output(AT91_PIN_PD8, 0);	//default receive
// end
}
...
// Start
#ifndef DDU //BCP
	at91_set_gpio_output(AT91_PIN_PD5, 0);	// DDU default receive / BCP default RFID off
	at91_set_gpio_output(AT91_PIN_PD6, 0);	// DDU default receive / BCP default RFID off
#endif
// end
}

// Start, add External UART structure
#ifdef DDU //DDU
// Quard UART A-D
#define UARTA_OFFSET 0x70000000
#define UARTB_OFFSET (0x70000000|(1<<22))
#define UARTC_OFFSET (0x70000000|(1<<21))
#define UARTD_OFFSET (0x70000000|(1<<22)|(1<<21))
// Dual UART E-F
#define UARTE_OFFSET 0x80000000
#define UARTF_OFFSET (0x80000000|(1<<20))
#else //BCP
#define UARTA_OFFSET 0x70000000
#endif

static struct resource ext_uart_resource[] = {
#ifdef DDU //DDU
	[0] = {
		.start	= UARTA_OFFSET,
		.end	= UARTA_OFFSET + 16,
...
static struct plat_serial8250_port ext_uart_ports[] = {
#ifdef DDU //DDU
	[0] = {
		.uartclk = 115200*120, /*115211*/
		.iotype = UPIO_MEM,
		.mapbase= UARTA_OFFSET,
	...
	[5] = {
		.uartclk = 115200*120, /*115211*/
		.iotype = UPIO_MEM,
		.mapbase= UARTF_OFFSET,
		.flags	= UPF_BOOT_AUTOCONF | UPF_IOREMAP ,
		.irq	= AT91_PIN_PE27,
	},
#else //BCP
	[0] = {
		.uartclk = 115200*120, /*115200*/
		//.uartclk = 230400*56, /*230400*/,
		.iotype = UPIO_MEM,
		.mapbase= UARTA_OFFSET,
...
static struct platform_device ext_uart_devices = {
        .name           = "serial8250",
        .id             = PLAT8250_DEV_PLATFORM,
		.num_resources	= ARRAY_SIZE(ext_uart_resource),
		.resource		= ext_uart_resource,
        .dev            = {
		        .platform_data  = ext_uart_ports,
        },
};
// end
...
void __init at91_add_device_serial(void)
{
	...
	// add ext uart
	printk(KERN_INFO "add ext uart\n");
	platform_device_register(&ext_uart_devices);
	printk(KERN_INFO "end ext uart\n");
	// end
...
// Start
// init extra DUART chip
void __init urp2_add_ext_uart(void)
{
	unsigned long csa;

#ifdef DDU	
	// Set RESET pin output H
       	at91_set_gpio_output(AT91_PIN_PE13, 1); /* Quad UART */
        at91_set_gpio_output(AT91_PIN_PE14, 1); /* Dual UART */

        // Set IRQ input
        at91_set_gpio_input(AT91_PIN_PA14, 1); /* UARTA */
        at91_set_gpio_input(AT91_PIN_PA15, 1); /* UARTB */
        at91_set_gpio_input(AT91_PIN_PE15, 1); /* UARTC */
        at91_set_gpio_input(AT91_PIN_PE16, 1); /* UARTD */
        at91_set_gpio_input(AT91_PIN_PE17, 1); /* UARTE */
        at91_set_gpio_input(AT91_PIN_PE27, 1); /* UARTF */

        // Disable RS232 Full Modem
        at91_set_gpio_output(AT91_PIN_PA6, 0);

#else	// BCP
        // Set RESET pin output H
        at91_set_gpio_output(AT91_PIN_PE16, 1); /* Single UART */
#endif
	
printk(KERN_INFO "Config SMC1\n");
	/* Configure NBI1_NCS0 on SMC1 */
	at91_sys_write(AT91_SMC1_SETUP(0), 0x00000002);
	at91_sys_write(AT91_SMC1_PULSE(0), 0x0A0A0A06);
	at91_sys_write(AT91_SMC1_CYCLE(0), 0x000A000A);
	at91_sys_write(AT91_SMC1_MODE(0), AT91_SMC_READMODE | AT91_SMC_WRITEMODE | AT91_SMC_EXNWMODE_DISABLE | AT91_SMC_TDF_(4) | AT91_SMC_DBW_8);

#ifdef DDU //DDU
	
	printk(KERN_INFO "Config SMC1\n");
	/* Configure NBI1_NCS1 on SMC1 */
	at91_set_B_periph(AT91_PIN_PE25, 0);		/* set NBI1_NCS1 B_PERIPH with NO PULL_UP */
	csa = at91_sys_read(AT91_MATRIX_EBI1CSA);	/* EBI1 Chip Select Assignment Register */
	at91_sys_write(AT91_MATRIX_EBI1CSA, csa | AT91_MATRIX_EBI1_CS1A_SMC); /* Chip Select 1 Assignment */
	
	at91_sys_write(AT91_SMC1_SETUP(1), 0x00000002);
	at91_sys_write(AT91_SMC1_PULSE(1), 0x0A0A0A06);
	at91_sys_write(AT91_SMC1_CYCLE(1), 0x000A000A);
	at91_sys_write(AT91_SMC1_MODE(1), AT91_SMC_READMODE | AT91_SMC_WRITEMODE | AT91_SMC_EXNWMODE_DISABLE | AT91_SMC_TDF_(4) | AT91_SMC_DBW_8);

	// Set RESET pin output L
	at91_set_gpio_output(AT91_PIN_PE13, 0); /* Quad UART */
	at91_set_gpio_output(AT91_PIN_PE14, 0); /* Dual UART */
        // Enable RS232 Full Modem
        at91_set_gpio_output(AT91_PIN_PA6, 1);

#else //BCP
	// Set RESET pin output L
	at91_set_gpio_output(AT91_PIN_PE16, 0); /* Single UART */
	// Set IRQ input
	at91_set_gpio_input(AT91_PIN_PE20, 0); /* UARTA */
	// Enable RS232 Full Modem
	at91_set_gpio_output(AT91_PIN_PE5, 1);
#endif
